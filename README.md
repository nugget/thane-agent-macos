...
- **Calendar provider** — EventKit-backed. Permission flow works. Events cross the wire in the zone they are scheduled in: timed events carry their own UTC offset plus an IANA `time_zone` when the event declares one, and all-day events carry inclusive `yyyy-MM-dd` dates resolved here against the event's own calendar. Zone and date arithmetic — DST transitions, both of EventKit's all-day end conventions — is covered by tests. Still narrow, and untested at scale: `list_events` and `create_event` only, no calendar enumeration, no update or delete, no attendees, and no per-calendar export policy (tracked upstream in [thane-ai-agent#1017](https://github.com/nugget/thane-ai-agent/issues/1017))
